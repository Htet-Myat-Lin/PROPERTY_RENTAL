          ...
          <Box p="4" borderWidth="1px" borderRadius="md">
            ...
            <Stack gap="2">
              <Flex justify="space-between">
                ...
                <Text>
                  {formData.utilityFee.electricity?.type} {" / "}
                  {formData.utilityFee.electricity?.amount && `${formData.utilityFee.electricity.amount}`}
                </Text>
              </Flex>
              <Flex justify="space-between">
                <Text fontWeight="medium">Water:</Text>
                <Text>
                  {formData.utilityFee.water?.type} {" / "}
                  {formData.utilityFee.water?.amount && `${formData.utilityFee.water.amount}`}
                </Text>
              </Flex>
              {formData.utilityFee.internet && (
                <Flex justify="space-between">
                  <Text fontWeight="medium">Internet (Utility):</Text>
                  <Text>
                    {formData.utilityFee.internet.type} {" / "}
                    {formData.utilityFee.internet.amount && `${formData.utilityFee.internet.amount}`}
                  </Text>
                </Flex>
              )}
              {formData.utilityFee.trashCollection && (
                <Flex justify="space-between">
                  <Text fontWeight="medium">Trash Collection:</Text>
                  <Text>
                    {formData.utilityFee.trashCollection.type} {" / "}
                    {formData.utilityFee.trashCollection.amount && `${formData.utilityFee.trashCollection.amount}`}
                  </Text>
                </Flex>
              )}
            </Stack>
